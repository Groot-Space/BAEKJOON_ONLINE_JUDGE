...
def j_dfs(J_queue, dirs,f_visited, r , c):
    while len(J_queue) != 0:
        J = J_queue.popleft()
        for dir in dirs:
            J_y = J[0] + dir[0]
            ...
            if mp[J_y][J_x] != '#' and J_y < r and J_x < c and J_y >= 0 and J_x >= 0:
                tmp = f_visited[J[0]][J[1]] + 1
                if tmp < f_visited[J_y][J_x] or f_visited[J_y][J_x] == 0:
                    f_visited[J_y][J_x] = tmp
                    J_queue.append([J_y, J_x])
                    if J_y == 0 or J_y == r-1 or J_x == 0 or J_x == c-1:
                        return f_visited[J_y][J_x]
...
J_point = J_queue[0]
f_visited[J_point[0]][J_point[1]] = 1
if J_point[0] == 0 or J_point[0] == r - 1 or J_point[1] == 0 or J_point[1] == c - 1:
    print(1)
else:
    print(j_dfs(J_queue, dirs, f_visited, r, c))